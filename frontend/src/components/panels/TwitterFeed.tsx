...
// TODO: integrate /api/sentiment/tweets endpoint when available for real Twitter data
...
function FeedItem({ article }: { article: NewsArticle }) {
  return (
    <a
      href={article.url}
      target="_blank"
      rel="noopener noreferrer"
      className="glass-panel-inner block p-4 transition-colors hover:bg-surface-hover"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <h4 className="font-body text-sm font-bold text-text-primary leading-tight">
            {article.title}
          </h4>
...
function TwitterFeed() {
  ...
  return (
    <GlassPanel title="TWITTER / NEWS FEED" className="flex flex-col h-full">
      {isLoading && !data && <LoadingSkeleton variant="line" lines={8} />}
      {error && !data && (
        <ErrorState message="News feed unavailable" />
      )}
        ...
        <div className="flex flex-1 flex-col gap-3 min-h-0">
          ...
          <div className="flex-1 space-y-3 overflow-y-auto max-h-[600px] pr-1">
            ...
            {sortedArticles.map((article, idx) => (
              <FeedItem key={`${article.url}-${idx}`} article={article} />
            ))}
          </div>
